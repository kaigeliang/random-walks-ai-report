...
\newcommand{\supplementlinkbox}{%
\par\smallskip\noindent
\begingroup
\setlength{\fboxsep}{3pt}%
\fcolorbox{linkblue}{white}{%
\begin{minipage}{0.96\linewidth}
\centering
\textbf{Interactive supplement:}
...
concept is both mathematically precise and easy to explain.
\supplementlinkbox
\end{abstract}
...
how far a drunk man would have wandered after $n$ steps~\cite{pearson1905}.
Five years
earlier, Louis Bachelier had used essentially the same model to study
fluctuations on the Paris stock exchange in his Sorbonne thesis~\cite{bachelier1900}, and in
1905 Albert Einstein independently invoked it to predict the motion of
particles suspended in a fluid~\cite{einstein1905}~--- a result that Jean Perrin later turned
...
page B (gold). The labels $in=k$ report raw in-degree, showing that
PageRank depends on random-walk traffic rather than in-degree alone.}
\label{fig:pagerank}
\end{figure}
...
\end{equation}
until $x_{T}$ is indistinguishable from pure noise. Equivalently,
iterating the walk gives
\begin{equation}
x_{t}=\sqrt{\bar{\alpha}_{t}}\,x_{0}
+\sqrt{1-\bar{\alpha}_{t}}\,\epsilon,\qquad
\bar{\alpha}_{t}=\prod_{s\leq t}(1-\beta_{s}),
\end{equation}
so the accumulated variance $1-\bar{\alpha}_{t}$ is the DDPM analogue of
the diffusive growth in Eq.~\eqref{eq:VarSn}. The continuous-time
limit is Brownian motion, as described in
...
In compact form, the training objective maximises
\begin{equation}
\sum_{v_i\in \mathrm{walk}}\sum_{v_j\in N(v_i)}
\log p(v_j\mid v_i),
\end{equation}
where $N(v_i)$ is the context window around $v_i$ along the sampled
walk. Thus the embedding is trained not from arbitrary graph distance,
but from the local neighbourhoods visited by the random walker.
...
sophisticated AI systems. Natural next directions include non-stationary
step distributions, heavy-tailed walks with rare large jumps, and
quantum-walk variants that replace classical transition probabilities by
...
L.~Bachelier, ``Th\'{e}orie de la sp\'{e}culation,'' \emph{Annales
Scientifiques de l'\'{E}cole Normale Sup\'{e}rieure}, vol.~17,
pp.~21--86, 1900.

\bibitem{pearson1905}
K.~Pearson, ``The problem of the random walk,'' \emph{Nature}, vol.~72,
no.~1865, p.~294, 1905.

\bibitem{einstein1905}
A.~Einstein, ``\"{U}ber die von der molekularkinetischen Theorie der
W\"{a}rme geforderte Bewegung von in ruhenden Fl\"{u}ssigkeiten
suspendierten Teilchen,'' \emph{Annalen der Physik}, vol.~17,
pp.~549--560, 1905.
